...
class myGreeter sleep exit_after =
  let n = ref 0 in
object
...
let main() =
  ...
  Arg.parse
    [ "-host", Arg.String (fun s -> host := s),
      ...
      "-sleep", Arg.Set_int sleep,
      "<n>   Sleep n seconds before responding";

      "-exit-after", Arg.Set_int exit_after,
      "<n>   Exit after getting n requests";
  ...
  let servant = new myGreeter !sleep !exit_after in
